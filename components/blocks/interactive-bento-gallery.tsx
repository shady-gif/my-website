...
const MediaItem = ({ item, className, onClick }: { item: MediaItemType, className?: string, onClick?: () => void }) => {
    ...
    if (item.type === 'display-ad') {
        return (
            <div className={className}>
                <DisplayAd slot={item.adSlot} label={item.title} />
            </div>
        );
    }

    if (item.type === 'video-ad') {
        return (
            <div className={className}>
                <VideoAd label={item.title} tagUrl={item.adTagUrl} />
            </div>
        );
    }
...
const InteractiveBentoGallery: React.FC<InteractiveBentoGalleryProps> = ({ mediaItems, title, description }) => {
    ...
    const isAdItem = (item: MediaItemType) => item.type === 'display-ad' || item.type === 'video-ad';
    const modalItems = items.filter((item) => !isAdItem(item));
                    ...
                    >
                        {items.map((item, index) => (
                            <motion.div
                                ...
                                className={`relative overflow-hidden rounded-xl ${isAdItem(item) ? 'cursor-default' : 'cursor-move'} ${item.span}`}
                                onClick={() => !isDragging && !isAdItem(item) && setSelectedItem(item)}
                                variants={{
